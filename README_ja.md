...
（デスクトップ反復、手動 `cargo ndk` / Gradle、署名）: [`docs/guides/build-and-release.md`](docs/guides/build-and-release.md)。
...
> **ターミナル不要ならエディタから。** アドオンを有効化すると、左パネルの **`XREAL Import`** dock が同じ
> vendoring を行います。*パッケージを選択…* をクリックして `com.xreal.xr(.tgz|.tar.gz)`（または展開済みの
> `package/` フォルダ）を選ぶと、システムの `tar` で展開し同じファイルを配置します。配置するのは XREAL
> 純正ライブラリのみ — アドオン本体の `libgodot_xreal.so` は従来どおり `cargo ndk` ビルド（またはプリビルト）
> から入ります。

このスクリプト（および dock）が Android エクスポートに必要なものをすべて配置します（配置先はすべて git 管理外。
