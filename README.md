...
| **Multi-resume** — glasses app keeps running when the phone switches apps | ⚠️ partial | The app **process** stays alive in the background (manifest scaffolding `nr_features=multiResume` + `NRFakeActivity`; the companion Activity stays resumed on the glasses display), and a floating "return" button is **implemented + device-verified** (`XrealFloatingReturnButton`: a Unity-free `TYPE_APPLICATION_OVERLAY` window — drag-to-move, tap-to-return, gated on `multiResume`, no AAR — see `docs/archive/codex-floatingmanager-analysis.md`). **But the glasses stop *rendering* while the app is backgrounded** (Godot pauses its render thread on `onPause` → `EGL_BAD_SURFACE`) and resume on return — under investigation (the earlier "rendering keeps updating after Home" claim is in doubt). |
