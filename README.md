...
Verified on the XREAL One Pro with the XREAL SDK for Unity 3.1.0 native libraries; rows marked
"Air 2 Ultra" were verified on the XREAL Air 2 Ultra. Every feature described on this page is
community-reverse-engineered interop, not an official API. The design notes and measurements behind each row live in the
[developer docs](docs/develop/README.md).

| Feature | Status | Notes |
|---|---|---|
| **Godot XR workflow** (`XROrigin3D`, `XRCamera3D`, `XRController3D`, `XRHandTracker`, InputMap) | ✅ | Your scene owns the hierarchy; the addon attaches to it. Controller inputs use Godot's standard OpenXR action names (`trigger_click`, `grip_click`, `menu_button`, `primary_click`, `trigger`, `grip`, `primary`). The XREAL-only capabilities in the rows that follow stay in this addon's own components. |
...
| **Tracking mode** 6DoF / 3DoF / 0DoF | ✅ | Select it with `xreal/tracking_type` or `XrealSystem.set_tracking_type`. |
| **Stereo glasses display** (head-locked peek window) | ✅ | World-locked 3D through the glasses. Multipass, both eyes, is the default. |
| **Multiview** stereo (single-pass-instanced) | ✅ Vulkan Mobile only; gains depend on content | Multipass works on both renderers and stays the default. Only the Vulkan Mobile renderer has true single-pass multiview: enable the `xreal/xr_multiview_poc` project setting (or `setprop debug.xreal.xr_multiview 1`) and a custom `XRInterfaceExtension` has Godot render both eyes in one scene pass into a two-layer target. On device it halves draw calls: a draw-call-bound scene ran 5.9% faster and a 100k-splat 3DGS scene slightly faster, while a GPU-bound scene ran 13% slower on the Adreno 710. It needs `xr/shaders/enabled=true` and XR Mode `OpenXR` on the export preset; the [addon README](addons/godot_xreal/README.md#project-settings) covers the setup. On the Compatibility (GL) renderer the setting is ignored with a warning and Multipass runs instead. |
| **Vulkan Mobile renderer** | ✅ device-verified, the default | A second export preset, "Android Vulkan", runs the whole port on Godot's Forward Mobile Vulkan renderer and installs alongside the shipping Compatibility build. Glasses rendering, the RGB camera, and FPV streaming and recording all work on device, and the colors match the Compatibility build. The glasses render tear-free at 60 FPS. That sync needs the `VK_KHR_external_semaphore_fd` device extension, requested through the `rendering/rendering_device/vulkan/additional_device_extensions` project setting, which `project.godot` already lists. The export template therefore has to come from a Godot that offers that setting. Without the extension the glasses bridge stops rather than falling back, so Compatibility stays fully supported and is the one to pick there. Mobile is the default rendering method, and both Android presets export it. It also unlocks `RenderingDevice` and GPU compute, which the Compatibility renderer lacks, and unifies the render path with Android XR and Project Aura. |
...
| **Diagnostics** (session and tracking state, HMD clock, plugin version) | ✅ | Read from `XrealSystem`. `get_capabilities()` answers what the connected glasses support as one `Dictionary` of `bool`, so code that needs the whole picture, such as a menu, replaces a getter per subsystem with a single call. |
...
| **Capture audio** (microphone and app audio) | ✅ | Recordings and FPV streams can carry both, and the SDK's encoder captures and mixes them natively; Godot's own mixer stays out of the path. Set the capture component's `audio_state`. The mic needs `RECORD_AUDIO`. App ("internal") audio needs an Android MediaProjection, because `addInternalAudio` makes the encoder open its own `AudioPlaybackCapture`: a screen-capture consent dialog appears on the first capture that asks for it, and that capture records mic-only while the next has both. For the DSP the mic goes through, see [what the microphone does and does not pick up](#what-the-microphone-does-and-does-not-pick-up). |
| **RGB camera** as a Godot `CameraFeed` | ✅ (One series) | Full color, shown in-scene on a head-locked quad. Runs alongside 6DoF, since SLAM uses the separate grayscale cameras. |
...
| **Phone controller → Godot XR/Input** | ✅ | `XrealXRRuntime` polls the NRController raw IMU/touchpad and `XrealXRInputRouter` fuses a standard `XRControllerTracker` pose. Each tracker publishes `aim`, `grip`, and `default`, the same pose set an OpenXR runtime publishes, so a plain `XRController3D` is aimed without setting its `pose` property. Glasses keys and app-owned phone UI buttons flow through the same addon bridge to `XRController3D` and `xr_select`/`xr_grab`/`xr_menu`. The demo only draws the standard pose as a ray. Raw native button bits remain unmapped until device-verified. |
...
The prebuilt addon needs no build of your own: download it and vendor the XREAL libraries.
...
2. Go to **Project > Project Settings > Plugins**, and enable **Godot XREAL**.
3. Vendor the XREAL runtime libraries; the **XREAL Import** dock does it in one click. See
   [Prerequisite](#prerequisite-vendor-the-xreal-runtime-libraries). They stay under XREAL's
...
files in the same git-ignored destinations, listed in the two tables in this section:

1. **Editor dock (recommended).** Go to **Project > Project Settings > Plugins** and enable **Godot
   XREAL**. Open the **XREAL Import** dock in the left panel, click **Select package…**, and pick
...
3. **Manual copy.** Extract the tgz yourself and copy the files in the two tables in this section
   into their destinations under the repo.

Vendoring handles only XREAL's proprietary libs; the addon's own `libgodot_xreal.so` still comes from
the `cargo ndk` build or a prebuilt release.
...
| `libVulkanSupport.so` | support lib that `libXREALNativeSessionManager.so` and `libXREALXRPlugin.so` need |
...
| `Log-Control-1.2.aar` | `LogControl`, referenced by `GlassesDisplayPlugEvent-2.4.2.aar`. Required: without it the app crashes before Godot starts | none |
...
> **Warning**: Never copy `nractivitylife*.aar`. Its launcher is Unity-only, and a Godot app that
> includes it fails to start.

This extension ignores the QNN/SNPE libs inside `nr_common.aar`, but they ride into the APK with the
aar.

## Build (from source)

The GDExtension is plain godot-rust: first vendor the XREAL libraries as described in
[Prerequisite](#prerequisite-vendor-the-xreal-runtime-libraries), then build. The full
...
### One script from build to launch
...
stages: cargo-ndk build, Godot APK export, `adb install`, launch. It first re-checks the
[vendored libraries](#prerequisite-vendor-the-xreal-runtime-libraries), both the four `.so` and the
addon's `.aar`/`.jar`, and prints the same guide if anything is missing.
...
2. Pick the renderer. **Mobile** is the default and renders the glasses tear-free at 60 FPS.
   Its Vulkan-to-glasses sync needs `VK_KHR_external_semaphore_fd`, requested through the
   `rendering/rendering_device/vulkan/additional_device_extensions` project setting. The export
   template therefore has to come from a Godot that offers that setting. Where it does not, the
   bridge stops rather than falling back, so use **Compatibility** instead. Forward+ is not an
   option: the session, head tracking, and the phone display come up while the glasses stay black.
...
This table lists the highlights. The [full class reference](docs/user/api/README.md) covers every class, method,
...
2. Click **Add image** for every reference image: choose the file and enter its physical printed width
   in meters (the dock generates a GUID). The SDK caps a set at five images. The dock rejects images with
...
3. Click **Build blob**. The tool runs and writes the `.bin` next to the manifest.
...
> **Warning**: Start the receiver on a PC on the same LAN before you click **Stream**. A receiver
> started afterwards has already missed the handshake, so the order cannot be reversed.

You never type an address. The app broadcasts, whichever receiver is listening answers, and
streaming begins.
...
page. Run it and click **Stream**: this port pairs with it exactly as the Unity SDK does.
...
Streaming uses the addon's own render target rather than the camera, so it needs no RGB camera and works on the
camera-less Air 2 Ultra.

#### What the microphone does and does not pick up
...
before you conclude something is broken. Every measurement in this list came from the device:

- A quiet room records as exact digital silence rather than a noise floor: every sample identical,
  `mean == max == -91 dB`. Make some noise before deciding the audio path is not working.
...
one track: an audio track 1.79 times the video's length, 35% of it silence. The SDK intends the
MediaProjection path described in the capture-audio row of
[Supported features](#supported-features).
...
scripts/                 build + vendor_xreal_libs + build_dummy_libs + build_image_db
                         + gen_stub_classes / gen_docs / gen_api_docs (.ps1/.sh pairs)
