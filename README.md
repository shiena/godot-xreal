...
| **Vulkan Mobile renderer** | ✅ device-verified, the default | A second export preset, "Android Vulkan", runs the whole port on Godot's Forward Mobile Vulkan renderer and installs alongside the shipping Compatibility build. Glasses rendering, the RGB camera, and FPV streaming and recording all work on device, and the colors match the Compatibility build. The glasses render tear-free, and the sync mode is selected at startup: 60 FPS with the `VK_KHR_external_semaphore_fd` device extension, about 52 FPS without it. Godot enables the extension through the `rendering/rendering_device/vulkan/additional_device_extensions` project setting, which `project.godot` already lists, so 60 FPS needs an export template from a Godot that offers that setting. Mobile is the default rendering method, and both Android presets export it. It also unlocks `RenderingDevice` and GPU compute, which the Compatibility renderer lacks, and unifies the render path with Android XR and Project Aura. |
...
2. Pick the renderer. **Mobile** is the default and renders the glasses tear-free. The sync mode
   is selected at startup: 60 FPS with the `VK_KHR_external_semaphore_fd` device extension, about
   52 FPS without it. Godot enables the extension through the
   `rendering/rendering_device/vulkan/additional_device_extensions` project setting, so 60 FPS
   needs an export template from a Godot that offers that setting. Forward+ is not an option:
   the session, head tracking, and the phone display come up while the glasses stay black.
