...
##
## The overlay is the SDK's real boundary polygon from GetPlaneBoundaryVertexData, which
## XREALPlaneSubsystem.cs advertises as supportsBoundaryVertices and the Unity sample draws. That
## is the actual detected outline, not an axis-aligned `center`/`size` bounds box. An on-device
## comparison picked the polygon: the bounds box, always at least as big as the surface, floated
## off the real edges, while the polygon tracks the true shape.
##
## World-locked: add this component under a world-fixed node, such as the scene root and NOT the
## head rig, so the overlays sit on the real surface as the head moves. Plane changes stream in
## through the shared XrealAR poller (XrealShared.get_ar), whose "planes" switch is gated on this
## toggle.
...
## Toggle the plane overlay and return the resulting state. It returns false when the plane ABI is
## unavailable on this device, so a UI toggle can flip itself back off. Planes then stream in
## through the shared XrealAR, which may take a moment and needs a live 6DoF session.
...
## Bring plane detection up; the call is idempotent. It returns false when the ABI is unavailable
## on this device.
func _start_detection() -> bool:
	if _system == null:
		return false
	# Gate on the ABI being resolved rather than on set_plane_detection_mode's return: the SDK
...
## Push the on/off state to the SDK and to the shared poller.
func _sync() -> void:
	if not _on:
		_planes.clear()
	if _ar:
...
## instead of waiting for the SDK's next change event. An established plane may not update for a
## while.
func _rebuild() -> void:
	for id in _planes:
		_update_plane(_planes[id])

## Resolve the shared XrealAR and connect its plane signals once, BEFORE the stream switch goes on,
...
## XrealAR signal (plane_added, plane_updated): overlay or refresh the plane. It logs the running
...
## Create or update the boundary-polygon overlay, the plane's real detected outline, fetched per
## plane from XrealSystem.get_plane_boundary(). The points are plane-local (X/Z) relative to the
## pose. Our Godot space is Unity's through M = diag(1,-1,-1), so a plane-local (x, 0, z) becomes
## (x, 0, -z).
...
## Shared translucent unshaded material, double-sided so it is visible from both faces and the
## polygon winding does not matter.
