...
Receives the Godot app's first-person-view stream: H.264 video *and* AAC audio over RTP, from the
...
## Requirements
- `python` 3, which answers the app's discovery broadcast. No `pip install`.
...
The server only reframes bytes: RTP → access units → FLV tags → WebSocket. Depacketizing takes
about 150 lines (RFC 6184 FU-A/STAP-A for video, RFC 3016 LATM for audio) and FLV muxing is a tag
header plus an `AVCDecoderConfigurationRecord` or `AudioSpecificConfig`. The browser's own H.264 and
AAC decoders do the rest, through [mpegts.js](https://github.com/xqq/mpegts.js/) (Apache-2.0) and
Media Source Extensions. `fpv_player.html` loads mpegts.js from a CDN; to run offline, vendor it
next to the page.
...
That stops either receiver, `fpv_server.py` included, since both share the same code. Stop one
...
The app has no "stream to this IP" field: it broadcasts and expects a peer to answer, the way
...
Both streams are plain standards, verified on device by decoding them rather than assumed:
...
- The audio track carries the microphone, the app's own output, or both, whichever the stream
  component's `audio_state` selects. The native encoder captures and mixes them itself, app audio
  through a MediaProjection. See the streaming section of the top-level README.
