...
> issue — `docs/develop/archive/codex-background-render-analysis.md`). PiP solves both (keeps rendering + its
> tile taps back), so `XrealFloatingReturnButton` was deleted and PiP shipped instead
> (`docs/develop/plans/background-render-plan.md`). The analysis below stands as the RE record; the "did not
