...
> `scripts/vendor_xreal_libs.ps1`/`.sh` (stages the `.so`/`.aar` — see `docs/develop/guides/build-and-release.md` and
> `docs/develop/plans/android-export-plugin-migration.md`). §3–§4 are kept as the reference for *what* must end
...
> `docs/develop/plans/frame-submission-plan.md`. Manifest-only gets you head tracking + XREAL mode;
...
6. Restore XREAL/NR Android AAR dependencies into the template.
   ...
   `scripts/vendor_xreal_libs.ps1` — see `docs/develop/guides/build-and-release.md`):
