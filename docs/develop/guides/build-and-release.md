...
1. **4 `.so` → `addons/godot_xreal/jniLibs/arm64-v8a/`** — three from
...
- **`release.yaml`** (`workflow_dispatch` with a `version` input) — bumps `plugin.cfg` + `Cargo.toml`
  ...
  `libgodot_xreal.so` plus a drop-in zip (`addons/godot_xreal/` with `godot_xreal.gdextension` and
  the built libraries inside, plus an `INSTALL.md`). Users unzip it and vendor the XREAL libs — no
  Rust / cargo-ndk needed. Changelog is generated by git-cliff (`.github/cliff.toml`) from
