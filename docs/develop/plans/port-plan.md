...
> place; per-feature docs live in `docs/develop/plans/` (see `docs/develop/README.md`). Main open item:
...
`docs/develop/reference/reverse-engineering.md` for why the native libraries are the integration surface.
...
- **Done (round a):** the perception ABI is RE-confirmed (signatures, `GetHMDTimeNanos` out-param,
  `LoadAPI` requirement) — see `docs/develop/reference/reverse-engineering.md`.
...
- **Done (round c, device-driven):** exposed as ONE process-global lazily-initialized session
  ...
  `docs/develop/guides/android-setup.md`.
...
- **Done (device verify, resolved):** (1) `CreateSession` succeeds with the Activity supplied
  ...
  glasses display mode directly (manifest markers, `docs/develop/guides/android-setup.md`) — no Nebula
  ...
  **w-leading** (w, x, y, z) — fixed with unit tests in `NrPose::to_godot_quaternion`
  ...
  (`head_pose_display`, full attitude incl. roll) — see `docs/develop/archive/roll-tracking-investigation.md`.
...
> displays on the XREAL One Pro. Winning recipe (see `docs/develop/plans/frame-submission-plan.md` + the project
