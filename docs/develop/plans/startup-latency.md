...
- **Java side**: call `XrealBridge.register` from `GodotApp.onCreate` (what the Java doc suggests)
  *and* drive the first `try_start()` from JNI, not from `_process`. Touches the Android build
  template, which is `skip-worktree`'d — see `docs/develop/plans/android-export-plugin-migration.md`.
...
`docs/develop/archive/codex-input-start-analysis.md`; the answer is not 6DoF SLAM and not our hand-built
`IUnityXRInput` table:
...
- The "before" column is a single cold start; the after-fix columns are three each. Absolute numbers
  move with SoC clock (see `docs/develop/plans/camera-feed-plan.md` for how much), so compare within a set.
