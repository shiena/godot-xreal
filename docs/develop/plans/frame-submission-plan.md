...
Status: **IMPLEMENTED — device-verified 2026-07-02 (Phase 2 achieved).** This is the
architecture that ships in `src/unity_plugin.rs`; the "verify on device" checklist at the bottom
all passed. Originally written as a plan derived from a clean disassembly pass of
`libXREALXRPlugin.so` (SDK v3.1.0, arm64-v8a) cross-checked against Unity's public XR SDK header
`IUnityXRDisplay.h`; it supersedes the trial-and-error probes recorded in
`docs/develop/reference/reverse-engineering.md` for the display path, and is kept as the ABI reference.
...
- Stereo ships as **Multi-pass (`stereo_rendering_mode = 0`) by default**; Multiview (mode 2) also
  ...
  `GL_TEXTURE_2D_ARRAY`" verdict (`docs/develop/archive/codex-righteye-analysis.md`) was disproved — the
...
- Eye textures are allocated **`GL_RGB10_A2` UNORM** (2026-07-21), matching Godot's
  ...
  sample time; 2026-07-17 A/B test, `docs/develop/archive/multiview-investigation.md`).
- `UpdateMetrics` inside `SubmitCurrentFrame` SIGBUSes on the null Unity metrics callback and is
  ...
  instead (`docs/develop/plans/render-metrics-gdscript-plan.md`).
