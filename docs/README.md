...
| [startup-latency.md](plans/startup-latency.md) | backoff fixed 2026-07-22; two leads open | **Why the glasses start rendering seconds after the phone screen.** Measured against the reference Unity app (+2.76 s to `GfxThreadStart` vs our +5.39 s). Fixed the bootstrap retry backoff (a flat 60-frame wait for an Activity that arrived in 145 ms) → 4.24 s. Records that **an autoload cannot fix the rest** — autoload `_ready()` beats the main scene but autoload `_process()` does not, so the first bootstrap attempt stays pinned to scene-load completion. Open: that ~1.45 s pin, and the SDK's input start taking 1.39 s against Unity's 0.20 s. |
