...
| [ar-features-plan.md](plans/ar-features-plan.md) | plane implemented; anchor/image/mesh RE'd | All 4 AR features' C ABI confirmed (codex RE). **Plane detection ported** (`XrealSystem.set_plane_detection_mode` / `poll_planes`; on-device verify pending). Anchor/image/mesh signatures + struct layouts documented for follow-up. |
