...
| [background-render-plan.md](plans/background-render-plan.md) | design, not implemented | Keep the glasses rendering while the app is backgrounded (multi-resume). The reparent is app-trivial; Godot only lacks Unity's detach-tolerant render thread → a ~15-line plain-Java `godot-lib` guard + app-side `removeView`/`addView` onto the resumed companion. Zero-patch validation: Picture-in-Picture (Milestone 0). |
