...
`CameraTexture` / shader can sample the live camera — the Godot-native equivalent of LayeredClient's
...
LayeredClient's `XrealFrameSource` uses **only plane 0 (Y)** as full-res grayscale for ORB matching.
...
- LayeredClient: `Assets/Layered/Scripts/Xreal/XrealFrameSource.cs`,
  `.../Core/IFrameSource.cs` (the frame-source abstraction we mirror).
